...
void sha1_raw (void* buf, unsigned int len, void* hash)
{
	sha1_ctx ctx;
	sha1_init_ctx (&ctx);
	sha1_process_bytes (buf, &ctx, len);
...
void sha1_update (void* ctx, void* buf, unsigned int len)
{
	sha1_process_bytes (buf, (sha1_ctx*) ctx, len);
}

void sha1_finish (void* ctx, void* hash)
{
	sha1_finish_ctx ((sha1_ctx*) ctx, (W8*) hash);
	free (ctx);
}
