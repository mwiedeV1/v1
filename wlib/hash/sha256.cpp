...
#define ALIGN4 _declspec(align(4))
#define UINT32 W32;
#else
...
struct sha256_ctx {
	...
	UINT32 total[2];
	UINT32 buflen;
	char buffer[128];   /* NB: always correctly aligned for UINT32.  */
};
...
sha256_finish_ctx (struct sha256_ctx *ctx, void *resbuf)
{
	...
	*(UINT32 *) &ctx->buffer[bytes + pad + 4] = SWAP (ctx->total[0] << 3);
	*(UINT32 *) &ctx->buffer[bytes + pad] = SWAP ((ctx->total[1] << 3) |
	                                        (ctx->total[0] >> 29));
...
sha256_process_bytes (const void *buffer, size_t len, struct sha256_ctx *ctx)
{
	...
	if (len >= 64) {
		/* To check alignment gcc has an appropriate operator.  Other
		   compilers don't.  */
#if __GNUC__ >= 2
# define UNALIGNED_P(p) (((uintptr_t) p) % __alignof__ (UINT32) != 0)
#else
# define UNALIGNED_P(p) (((uintptr_t) p) % sizeof (UINT32) != 0)
#endif
...
{
	ALIGN4 unsigned char alt_result[32];
	ALIGN4 unsigned char temp_result[32];
	...
	if ((key - (char *) 0) % sizeof (UINT32) != 0) {
		char *tmp = (char *) alloca (key_len + sizeof (UINT32));
		key = copied_key = (char*) memcpy (tmp + sizeof (UINT32) - (tmp - (char *) 0) % sizeof (UINT32),  key, key_len);
	}

	if ((salt - (char *) 0) % sizeof (UINT32) != 0) {
		char *tmp = (char *) alloca (salt_len + sizeof (UINT32));
		salt = copied_salt = (char*) memcpy (tmp + sizeof (UINT32) - (tmp - (char *) 0) % sizeof (UINT32), salt, salt_len);
	}
...
bool sha256 (WCSTR input, WCSTR salt, WString& output)
{
	char* ret = sha256_crypt ((WCSTR) input, (WCSTR) salt);
	if (ret) {
		output = (WCSTR) ret;
		free (ret);
		return true;
...
bool sh256_check (WCSTR input, WCSTR enc)
{
	WString check;
	sha256 (input, enc, check);
...
void sha256_update (void* ctx, void* buf, unsigned int len)
{
	sha256_process_bytes (buf, len, (sha256_ctx*) ctx);
}

void sha256_finish (void* ctx, void* hash)
{
	sha256_finish_ctx ((sha256_ctx*) ctx, hash);
	free (ctx);
}
