...
install:
	...
	cd $(INST_DIR) && tar cvfz $(PROGRAM_NAME)-$(PROGRAM_VERSION)-$(SYSTEM)-$(ARCH).tar.gz --transform s/$(PROGRAM_NAME)/$(PROGRAM_NAME)-$(PROGRAM_VERSION)/ --format=gnu --exclude='$(PROGRAM_NAME)/*.ttf' --exclude='$(PROGRAM_NAME)/cert/*.crt' --exclude='$(PROGRAM_NAME)/cert/*.key' --exclude='$(PROGRAM_NAME)/app/chat*' --exclude='$(PROGRAM_NAME)/x11-examples/xt-*' --owner=1000 --group=1000 $(PROGRAM_NAME)
