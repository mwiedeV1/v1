...
int freeHandle (void* handle, int handletype) {
	...
	if (handletype == Handle::HANDLETYPE_DIRECTORY) {
		delete (WDirectoryEntries*) handle;
	}
	else
	if (handletype == Handle::HANDLETYPE_SOCKET) {
		delete (WSocket*) handle;
	}
	else
	if (handletype == Handle::HANDLETYPE_SERVERSOCKET) {
		delete (WServerSocket*) handle;
	}
	else
	if (handletype == Handle::HANDLETYPE_THREAD) {
...
int callbackA1 (void* arg1) {
    ...
    DataValue param1 = (double) *(uint64_t*) &arg1;
...
int callbackA2 (void* arg1, void* arg2) {
    ...
    DataValue param1 = (double) *(uint64_t*) &arg1;
    argvalues.push_back (&param1);
    DataValue param2 = (double) *(uint64_t*) &arg2;
...
int callbackA3 (void* arg1, void* arg2, void* arg3) {
    ...
    DataValue param2 = (double) *(uint64_t*) &arg2;
    argvalues.push_back (&param2);
    DataValue param3 = (double) *(uint64_t*) &arg3;
...
int callbackA4 (void* arg1, void* arg2, void* arg3, void* arg4) {
    ...
    DataValue param3 = (double) *(uint64_t*) &arg3;
    argvalues.push_back (&param3);
    DataValue param4 = (double) *(uint64_t*) &arg4;
...
SCallbackProc g_callbackList[] = {
	{ "callbackA1", (uint64_t) callbackA1 },
	{ "callbackA2", (uint64_t) callbackA2 },
	{ "callbackA3", (uint64_t) callbackA3 },
	{ "callbackA4", (uint64_t) callbackA4 },
	{ "callbackB1", (uint64_t) callbackB1 },
	{ "callbackB2", (uint64_t) callbackB2 },
	{ "callbackB3", (uint64_t) callbackB3 },
	{ "callbackB4", (uint64_t) callbackB4 }
};
...
int func_print (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
  ...
  if (ctx.programMode != InterpreterContext::PROGRAM_MODE_WEB)
    fflush (stdout);
	ret = 1;
  return 0;
}
			...
			if (!fFormat) {
				...
				if (argvalues.size ()>argIdx)
				{
					switch (*c) {
						...
						case 'X':
							{
								long long int value = (long long int) (double) *argvalues[argIdx];
								...
								if (sscanf (c0, format.c_str (), &value)) {
                                    ...
                                    if (*c=='i' || *c=='d') {
                                      ...
                                      if (*cPrev=='h' && *cPrevPrev=='h')
                                          ...
                                          arg->numvalue = (double) (unsigned long long int) value;
...
int func_binparse (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
	...
	switch (type) {
		...
		case 6:
			...
			if (argvalues[0]->value.length ()>=(sizeof (uint64_t)+pos)) {
				if (fSigned)
					ret.numvalue = (double) *((int64_t*) (argvalues[0]->value.c_str ()+pos));
				else
					ret.numvalue = (double) *((uint64_t*) (argvalues[0]->value.c_str ()+pos));
				ret.datatype = DataValue::DATATYPE_NUM;
			}
			else {
				warnLen = sizeof (uint64_t);
			}
			break;
...
int func_binformat (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
	...
	switch (type) {
		...
		case 7:
			{
				// Pointer / System default
				uint64_t bin = (double) argvalues[0]->numvalue;
				ret.value.assign ((const char*) &bin, 8);
			}			
			break;
	...
	try {
		...
		if (process.execute (argvalues[0]->value.c_str (), (void*) (envAllStr.length ()>0 ? (WCSTR) envAllStr : NULL))) {
            ...
            bytesWritten=argvalues[3]->value.size ();
				...
				if (argvalues[3]->datatype==DataValue::DATATYPE_HANDLE) {
...
int func_fopen (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
	...
	try {
		...
		if (argvalues[0]->value=="v1://stdout")
			((WFile*) *h)->open (stdout, false, false);		
		else
		if (argvalues[0]->value=="v1://stderr")
			((WFile*) *h)->open (stderr, false, false);		
		else
		if (fCheck && !WSystem::existsFile (argvalues[0]->value.c_str ())) {
...
int func_fwrite (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
	...
	try {
		if (h && ctx.handleHT.isKey (h)) {
			...
			if (h->handletype==Handle::HANDLETYPE_SOCKET) {
...
int func_filetype (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
	...
	if (WSystem::existsFile (argvalues[0]->value.c_str (), false)) {
		ret.datatype=DataValue::DATATYPE_STR;
		ret.value="file";
	}
  else
	...
	while (arrayList.hasMoreElements ()) 
    {
		...
		*/
		...
		else 
		{
			...
			if (value->datatype==DataValue::DATATYPE_BOOL) {
...
int func_in_array_universal (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx, DataValue* key=NULL) {
	...
	while (argvalues[1]->arrayList.hasMoreElements ()) {
		...
		if (argvalues[0]->datatype<DataValue::DATATYPE_ARRAY) {
			...
			if (argvalues[0]->datatype==value->datatype) {
				if (argvalues[0]->datatype!=DataValue::DATATYPE_STR)
...
int func_universal_call (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx, void* func, const int* paramDef, unsigned int paramCnt=0, int retDef=0) {
	...
	for (unsigned int i=3;i<(paramCnt+3);i++) {
		...
		if ((type>=CALL_TYPE_BOOL) && (type<=CALL_TYPE_POINTER)) {
			if (argvalues.size ()<(i+1) || argvalues[i]->datatype>DataValue::DATATYPE_STR || argvalues[i]->datatype==DataValue::DATATYPE_NO_PARAM) {
				return ((i+1)<<8)|WSCRIPT_RET_NUM_REQUIRED;
			}
		}
		else
		if (argvalues.size ()<(i+1) || argvalues[i]->datatype<DataValue::DATATYPE_ARRAY) {
			return ((i+1)<<8)|WSCRIPT_RET_ARRAY_REQUIRED;
		}		
		else {
			arrayParamIdx = i;
	...
	for (int x=0;x<paramCnt;x++ && x2<20) {
		...
		if (type==CALL_TYPE_REFERENCE) {
        	...
        	if (type==CALL_TYPE_POINTER) {
                v = (void*) (uint64_t) argvalues[x+3]->numvalue;
                voidList[x2++]=v;
        	}
		else
		if (type==CALL_TYPE_INT) {
			v = (void*) (int) argvalues[x+3]->numvalue;
			voidList[x2++]=v;
		}
		else
		if (type==CALL_TYPE_FLOAT) {

            switch (mmxReg) {
              case 0: __asm__ __volatile__ ("movsd %0,%%xmm0;cvtpd2ps %%xmm0, %%xmm0"::"m"(argvalues[x+3]->numvalue):); break;
              case 1: __asm__ __volatile__ ("movsd %0,%%xmm1;cvtpd2ps %%xmm1, %%xmm1"::"m"(argvalues[x+3]->numvalue):); break;
              case 2: __asm__ __volatile__ ("movsd %0,%%xmm2;cvtpd2ps %%xmm2, %%xmm2"::"m"(argvalues[x+3]->numvalue):); break;
              case 3: __asm__ __volatile__ ("movsd %0,%%xmm3;cvtpd2ps %%xmm3, %%xmm3"::"m"(argvalues[x+3]->numvalue):); break;
              case 4: __asm__ __volatile__ ("movsd %0,%%xmm4;cvtpd2ps %%xmm4, %%xmm4"::"m"(argvalues[x+3]->numvalue):); break;
              case 5: __asm__ __volatile__ ("movsd %0,%%xmm5;cvtpd2ps %%xmm5, %%xmm5"::"m"(argvalues[x+3]->numvalue):); break;
              case 6: __asm__ __volatile__ ("movsd %0,%%xmm6;cvtpd2ps %%xmm6, %%xmm6"::"m"(argvalues[x+3]->numvalue):); break;
              case 7: __asm__ __volatile__ ("movsd %0,%%xmm7;cvtpd2ps %%xmm7, %%xmm7"::"m"(argvalues[x+3]->numvalue):); break;
              default:
		...
		if (type==CALL_TYPE_DOUBLE) {
			
			switch (mmxReg) {
              case 0: __asm__ __volatile__ ("movsd %0,%%xmm0"::"m"(argvalues[x+3]->numvalue):); break;
              case 1: __asm__ __volatile__ ("movsd %0,%%xmm1"::"m"(argvalues[x+3]->numvalue):); break;
              case 2: __asm__ __volatile__ ("movsd %0,%%xmm2"::"m"(argvalues[x+3]->numvalue):); break;
              case 3: __asm__ __volatile__ ("movsd %0,%%xmm3"::"m"(argvalues[x+3]->numvalue):); break;
              case 4: __asm__ __volatile__ ("movsd %0,%%xmm4"::"m"(argvalues[x+3]->numvalue):); break;
              case 5: __asm__ __volatile__ ("movsd %0,%%xmm5"::"m"(argvalues[x+3]->numvalue):); break;
              case 6: __asm__ __volatile__ ("movsd %0,%%xmm6"::"m"(argvalues[x+3]->numvalue):); break;
              case 7: __asm__ __volatile__ ("movsd %0,%%xmm7"::"m"(argvalues[x+3]->numvalue):); break;
              default:	
                  ctx.abortInterprete ("Too many floating point parameters (8 allowed)");
                  return 0;
                  break;
	...
	if (retDef==CALL_TYPE_BOOL) {		
		ret = (bool) (v3!=0);
	}
	else
	if (retDef==CALL_TYPE_STRING) {	
		ret = (WCSTR) v3;
	}
	else
	if (retDef==CALL_TYPE_POINTER) {	
		ret = (double) v3;
	}
	else
	if (retDef==CALL_TYPE_FLOAT) {
		float f;
		__asm__ __volatile__ ("movss %%xmm0,%0":"=m"(f)::);
		ret = (double) f;        
	}
	else
    if (retDef==CALL_TYPE_DOUBLE) {
        __asm__ __volatile__ ("movq %%xmm0, %0":"=r"(v2):);
		ret = (double) v2;
    }
	else {
		ret = true;
...
int func_dlsym (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
	...
	if (PARAM1_NOSTR) {
        ...
        HMODULE hDLL = (void*) (uint64_t) argvalues[0]->numvalue;
    if (argvalues.size ()<2 || argvalues[1]->datatype>DataValue::DATATYPE_STR) {
                return WSCRIPT_RET_PARAM2|WSCRIPT_RET_STR_REQUIRED;
        }
	...
	if (hDLL==NULL) {
		...
		for (int i=0;i<sizeof (g_callbackList)/sizeof (SCallbackProc);i++) {
			if (argvalues[1]->value==g_callbackList[i].name) {
				ret = (double) *(uint64_t*) &g_callbackList[i].procPtr;
				break;
			}
		}
		if (!ret)
...
int func_call (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
	...
	HMODULE hDLL = (void*) (uint64_t) argvalues[0]->numvalue;
	// memcpy (&hDLL, &argvalues[0]->numvalue, sizeof (hDLL)); // Copy without loss data
    // printf ("%08x\r\n", hDLL);
    if (argvalues.size ()<2 || argvalues[1]->datatype>DataValue::DATATYPE_STR) {
		return WSCRIPT_RET_PARAM2|WSCRIPT_RET_STR_REQUIRED;
	}
	...
	if (argvalues[1]->datatype==2) {
		procAddr = (void*) (uint64_t) argvalues[1]->numvalue;
		argvalues[1]->toString ();		
	}
	else {
	...
	if (argvalues.size ()>2) {
		if (argvalues[2]->datatype<=DataValue::DATATYPE_STR)
		{
			...
			while (*c!=0 && paramCnt<(sizeof(paramDef)/sizeof(int))) {
				...
				{
					switch (*(c+1)) {
						case 'i': retDef = CALL_TYPE_INT; break;
						case 'b': retDef = CALL_TYPE_BOOL; break;
						case 'f': retDef = CALL_TYPE_FLOAT; break;					
						case 'd': retDef = CALL_TYPE_DOUBLE; break;					
						case 's': retDef = CALL_TYPE_STRING; break;
						case 'p': retDef = CALL_TYPE_POINTER; break;
					}
					break;
				}
				switch (*c) {
					case 'f': paramDef[paramCnt++] = CALL_TYPE_FLOAT; break;
					case 'd': paramDef[paramCnt++] = CALL_TYPE_DOUBLE; break;
					case 's': paramDef[paramCnt++] = CALL_TYPE_STRING; break;
					case 'p': paramDef[paramCnt++] = CALL_TYPE_POINTER; break;
					case 'r': paramDef[paramCnt++] = CALL_TYPE_REFERENCE; break;
					case 'a': paramDef[paramCnt++] = CALL_TYPE_INT_ARRAY; break;
					default:
...
int func_dl (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
    ...
    if (!ext.equals (""))
      ...
      ext = ".so";    
    hDLL = dlopen((WCSTR) WFormattedString ("%s%s%s", (WCSTR) currDir, argvalues[0]->value.c_str (), (WCSTR) ext),  RTLD_LAZY);
    if (hDLL!=NULL) { 
        if (fNative) 
        {
            // memcpy (&ret.numvalue, &hDLL, sizeof (hDLL)); // Copy without loss data
            // ret.datatype=DataValue::DATATYPE_NUM;             
            ret = (double) (uint64_t) hDLL;	    
        }
        else {
...
int func_memat (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
	...
	void* memory  = (void*) (uint64_t) argvalues[0]->numvalue;
...
int func_memref (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
	...
	if (argvalues[0]->refPnt)
		ret = (double) (uint64_t) argvalues[0]->refPnt->value.c_str ();
	else
		ret = (double) (uint64_t) argvalues[0]->value.c_str ();
	return 0;
}
...
int func_memset (vector<DataValue*>& argvalues, DataValue& ret, InterpreterContext& ctx) {
	...
	char* memPnt = (char*) (uint64_t)  argvalues[0]->numvalue + pos;

	memcpy (memPnt, argvalues[1]->value.c_str (), argvalues[1]->value.size ());
	ret = argvalues[1]->value.size ();
	return 0;
}
...
DataValue* Expression::calc (InterpreterContext& ctx, DataValue& retvalue, unsigned int flags) 
{
		...
		for (unsigned int idx = 0;idx<arguments.size(); idx++) 
		{
			...
			if (opCode & OPCODE_EQUAL_OP) {
				if (idx==0) {				
					...
					if ((aPnt->flags&DataValue::FLAGS_USEREF) && aPnt->refPnt)
						aPnt = aPnt->refPnt;					
					continue;
				}
				if (idx<=indexCnt) {
	...
	if (datatype>1)
	{
		...
		if (datatype==DataValue::DATATYPE_ARRAYDEF) 
		{
			// Array definition
			int idx = 0;
			bool fOwnKeys = false;
		...
		if (datatype>=DataValue::DATATYPE_ARRAY) {
				...
				if (j==0) {
					...
					for (unsigned int i = 0;i<arguments.size(); i++) 
					{
						...
						if (indexCnt>0) {
							...
							if (flags==EXPRESSION_FLAG_STORE) {
								if (&retvalue!=arrayValue) { // Dont assign myself (problems with arrays!)
									if (retvalue.datatype>=DataValue::DATATYPE_ARRAY)
	...
	if (_opCode==OPCODE_EQUAL)
	{
		...
		if (!ctx.symbols[0]->isKey (constvalue->c_str ()))
		{
			// Variable
			...
			retValuePnt->refPnt = retValuePnt; 
			if (flags==EXPRESSION_FLAG_STORE) {
				if (&retvalue!=retValuePnt) { // Dont assign myself (problems with arrays!)
					if (retvalue.datatype>=DataValue::DATATYPE_ARRAY)
